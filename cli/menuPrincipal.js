...
export async function menuPrincipal(base, cliente) {
    ...
    while (salirMain == false) {
        const { opcion } = await opciones("Clientes", "Planes de Entrenamiento", "Seguimiento", "Contratos", "Gestión Financiera", "Salir")
        console.log(opcion)
        switch (opcion) {
            case "Clientes":
...
async function subMenuGestion() {
    ...
    while (!exitGestion) {
        const { opcionGestion } = await opciones("Crear Movimiento", "Consultar Historial de Movimientos", "Balance por clientes", "Balance general", "Volver al menu anterior")
